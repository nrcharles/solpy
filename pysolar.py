...
#    for i in range(48):
#        timestamp = d.ctime()
#        altitude_deg = Altitude(latitude_deg, longitude_deg, d)
#        azimuth_deg = Azimuth(latitude_deg, longitude_deg, d)
#        power = radiation.RadiationDirect(d, altitude_deg)
#        if (altitude_deg > 0):
...
def AberrationCorrection(radius_vector):     # r is earth radius vector [astronomical units]
    return -20.4898/(3600.0 * radius_vector)

def Altitude(latitude_deg, longitude_deg, utc_datetime, elevation = 0, temperature_celsius = 25, pressure_millibars = 1013.25):
    '''See also the faster, but less accurate, AltitudeFast()'''
    # location-dependent calculations    
    projected_radial_distance = ProjectedRadialDistance(elevation, latitude_deg)
    ...
    jme = julian.JulianEphemerisMillenium(jce)
    geocentric_latitude = GeocentricLatitude(jme)
    geocentric_longitude = GeocentricLongitude(jme)
    radius_vector = RadiusVector(jme)
    aberration_correction = AberrationCorrection(radius_vector)
    equatorial_horizontal_parallax = EquatorialHorizontalParallax(radius_vector)
    nutation = Nutation(jde)
    apparent_sidereal_time = ApparentSiderealTime(jd, jme, nutation)
    true_ecliptic_obliquity = TrueEclipticObliquity(jme, nutation)
    
    # calculations dependent on location and time
    apparent_sun_longitude = ApparentSunLongitude(geocentric_longitude, nutation, aberration_correction)
    geocentric_sun_right_ascension = GeocentricSunRightAscension(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude)
    geocentric_sun_declination = GeocentricSunDeclination(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude)
    local_hour_angle = LocalHourAngle(apparent_sidereal_time, longitude_deg, geocentric_sun_right_ascension)
    parallax_sun_right_ascension = ParallaxSunRightAscension(projected_radial_distance, equatorial_horizontal_parallax, local_hour_angle, geocentric_sun_declination)
    topocentric_local_hour_angle = TopocentricLocalHourAngle(local_hour_angle, parallax_sun_right_ascension)
    ...
    topocentric_elevation_angle = TopocentricElevationAngle(latitude_deg, topocentric_sun_declination, topocentric_local_hour_angle)
    refraction_correction = RefractionCorrection(pressure_millibars, temperature_celsius, topocentric_elevation_angle)
    return topocentric_elevation_angle + refraction_correction

def AltitudeFast(latitude_deg, longitude_deg, utc_datetime):

# expect 19 degrees for solar.Altitude(42.364908,-71.112828,datetime.datetime(2007, 2, 18, 20, 13, 1, 130320))

    day = DayOfYear(utc_datetime)
    declination_rad = math.radians(Declination(day))
    latitude_rad = math.radians(latitude_deg)
    hour_angle = HourAngle(utc_datetime, longitude_deg)
...
def ApparentSiderealTime(julian_day, jme, nutation):
    return MeanSiderealTime(julian_day) + nutation['longitude'] * math.cos(TrueEclipticObliquity(jme, nutation))

def ApparentSunLongitude(geocentric_longitude, nutation, ab_correction):
    return geocentric_longitude + nutation['longitude'] + ab_correction

def Azimuth(latitude_deg, longitude_deg, utc_datetime, elevation = 0):
    ...
    topocentric_local_hour_angle = TopocentricLocalHourAngle(local_hour_angle, parallax_sun_right_ascension)
    topocentric_sun_declination = TopocentricSunDeclination(geocentric_sun_declination, projected_axial_distance, equatorial_horizontal_parallax, parallax_sun_right_ascension, local_hour_angle)
    return 180 - TopocentricAzimuthAngle(topocentric_local_hour_angle, latitude_deg, topocentric_sun_declination)

def AzimuthFast(latitude_deg, longitude_deg, utc_datetime):
# expect -50 degrees for solar.Azimuth(42.364908,-71.112828,datetime.datetime(2007, 2, 18, 20, 18, 0, 0))
    day = DayOfYear(utc_datetime)
    declination_rad = math.radians(Declination(day))
    latitude_rad = math.radians(latitude_deg)
    hour_angle_rad = math.radians(HourAngle(utc_datetime, longitude_deg))
    altitude_rad = math.radians(Altitude(latitude_deg, longitude_deg, utc_datetime))
...
def Coefficient(jme, constant_array):
...
def EquatorialHorizontalParallax(radius_vector):
    return 8.794 / (3600 / radius_vector)

def FlattenedLatitude(latitude):
...
def GeocentricLatitude(jme):
    return -1 * HeliocentricLatitude(jme)

def GeocentricLongitude(jme):
    return (HeliocentricLongitude(jme) + 180) % 360

def GeocentricSunDeclination(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude):
...
def GeocentricSunRightAscension(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude):
    apparent_sun_longitude_rad = math.radians(apparent_sun_longitude)
    true_ecliptic_obliquity_rad = math.radians(true_ecliptic_obliquity)
    geocentric_latitude_rad = math.radians(geocentric_latitude)
...
def HeliocentricLatitude(jme):
    b0 = Coefficient(jme, constants.B0)
    b1 = Coefficient(jme, constants.B1)
    return math.degrees((b0 + (b1 * jme)) / 10 ** 8)

def HeliocentricLongitude(jme):
    l0 = Coefficient(jme, constants.L0)
    l1 = Coefficient(jme, constants.L1)
    l2 = Coefficient(jme, constants.L2)
    l3 = Coefficient(jme, constants.L3)
    l4 = Coefficient(jme, constants.L4)
    l5 = Coefficient(jme, constants.L5)

    l = (l0 + l1 * jme + l2 * jme ** 2 + l3 * jme ** 3 + l4 * jme ** 4 + l5 * jme ** 5) / 10 ** 8
    return math.degrees(l) % 360

def HourAngle(utc_datetime, longitude_deg):
    solar_time = SolarTime(longitude_deg, utc_datetime)
    return 15 * (12 - solar_time)

def IncidenceAngle(topocentric_zenith_angle, slope, slope_orientation, topocentric_azimuth_angle):
...
def Nutation(jde):
    ...
    for i in range(len(abcd)):
        sigmaxy = NutationAberrationXY(jce, i, x)
...
def ParallaxSunRightAscension(projected_radial_distance, equatorial_horizontal_parallax, local_hour_angle, geocentric_sun_declination):
...
def ProjectedRadialDistance(elevation, latitude):
    flattened_latitude_rad = math.radians(FlattenedLatitude(latitude))
    latitude_rad = math.radians(latitude)
    return math.cos(flattened_latitude_rad) + (elevation * math.cos(latitude_rad) / constants.earth_radius)

def ProjectedAxialDistance(elevation, latitude):
    flattened_latitude_rad = math.radians(FlattenedLatitude(latitude))
    latitude_rad = math.radians(latitude)
    return 0.99664719 * math.sin(flattened_latitude_rad) + (elevation * math.sin(latitude_rad) / constants.earth_radius)

def RadiusVector(jme):
    r0 = Coefficient(jme, constants.R0)
    r1 = Coefficient(jme, constants.R1)
    r2 = Coefficient(jme, constants.R2)
    r3 = Coefficient(jme, constants.R3)
    r4 = Coefficient(jme, constants.R4)

    return (r0 + r1 * jme + r2 * jme ** 2 + r3 * jme ** 3 + r4 * jme ** 4) / 10 ** 8

def RefractionCorrection(pressure_millibars, temperature_celsius, topocentric_elevation_angle):
...
def SolarTime(longitude_deg, utc_datetime):
    day = DayOfYear(utc_datetime)
...
def TopocentricAzimuthAngle(topocentric_local_hour_angle, latitude, topocentric_sun_declination):
...
def TopocentricElevationAngle(latitude, topocentric_sun_declination, topocentric_local_hour_angle):
    latitude_rad = math.radians(latitude)
    tsd_rad = math.radians(topocentric_sun_declination)
    tlha_rad = math.radians(topocentric_local_hour_angle)
...
def TopocentricLocalHourAngle(local_hour_angle, parallax_sun_right_ascension):
    return local_hour_angle - parallax_sun_right_ascension

def TopocentricSunDeclination(geocentric_sun_declination, projected_axial_distance, equatorial_horizontal_parallax, parallax_sun_right_ascension, local_hour_angle):
    gsd_rad = math.radians(geocentric_sun_declination)
...
def TopocentricSunRightAscension(projected_radial_distance, equatorial_horizontal_parallax, local_hour_angle,
        apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude):
    gsd = GeocentricSunDeclination(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude)
    psra = ParallaxSunRightAscension(projected_radial_distance, equatorial_horizontal_parallax, local_hour_angle, gsd)
    gsra = GeocentricSunRightAscension(apparent_sun_longitude, true_ecliptic_obliquity, geocentric_latitude)
    return psra + gsra

def TopocentricZenithAngle(latitude, topocentric_sun_declination, topocentric_local_hour_angle, pressure_millibars, temperature_celsius):
    tea = TopocentricElevationAngle(latitude, topocentric_sun_declination, topocentric_local_hour_angle)
    return 90 - tea - RefractionCorrection(pressure_millibars, temperature_celsius, tea)

def TrueEclipticObliquity(jme, nutation):
    u = jme/10.0
